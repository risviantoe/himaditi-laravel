...
<body>
    <div>
        <nav class="navbar fixed-top shadow-sm navbar-light">
            <div class="container-fluid">
                <span class="navbar-brand mb-0 h1">HIMADITI</span>
                <div class="nav-user d-flex align-items-center">
                    <div class="avatar">
                        @if (auth()->user()->gender === 'L')
                            <img src="{{asset('img/backend/avatar-cowo.svg')}}">
                        @else
                            <img src="{{asset('img/backend/avatar-cewe.svg')}}">
                        @endif
                    </div>
        ...
        <div class="d-flex">
            <div class="col-md-12 main">
                <div class="banner">
                    ...
                    <h4>Selamat Datang di Dashboard Admin HIMADITI</h4>
                </div>
                @yield('content')
            </div>
    ...
    <script>
        function sweetDel(ev) {
            ev.preventDefault();
            var target = ev.currentTarget.getAttribute('target'); //use currentTarget because the click may be on the nested i tag and not a tag causing the href to be empty
